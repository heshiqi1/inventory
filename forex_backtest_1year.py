"""
H1/H2 Price Action Strategy - 最近1年数据回测
使用日线数据回测 forex_monitor_wx.py 策略 (最近1年)
...
class Forex1YearBacktester:
    ...
    def __init__(self, symbol: str, start_date: str, end_date: str,
        ...
        self.df = None
        self.chart_output_dir = chart_output_dir or "backtest_charts_1year"
        
        if not os.path.exists(self.chart_output_dir):
            os.makedirs(self.chart_output_dir)

    def load_data(self) -> pd.DataFrame:
        """从 Stooq 加载数据（与 forex_monitor_wx.py 完全一致）"""
...
def run_1year_backtest():
    ...
    RISK_PER_TRADE = 0.02
    CHART_OUTPUT_DIR = "backtest_charts_1year"

    print("="*70)
    print("H1/H2 外汇/贵金属回测 (forex_monitor_wx.py 策略 - 最近1年)")
    print("="*70)
    print(f"时间周期: 日线")
    ...
    for name, symbol in SYMBOLS.items():
        ...
        backtester = Forex1YearBacktester(
            symbol=symbol,
