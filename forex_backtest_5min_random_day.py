...
class TradingSystem5MinBacktester:
    """基于交易系统规则的5分钟数据回测引擎 - 随机1天"""

    def __init__(self, symbol: str, initial_capital: float = 100000, 
                 risk_per_trade: float = 0.02, chart_output_dir: str = None):
        self.symbol = symbol
        self.initial_capital = initial_capital
        self.risk_per_trade = risk_per_trade
        self.df = None
        self.selected_date = None
        self.chart_output_dir = chart_output_dir or "backtest_charts_5min_random"
        
        if not os.path.exists(self.chart_output_dir):
            os.makedirs(self.chart_output_dir)

    def load_data_random_day(self, api_key: str, lookback_days: int = 60) -> pd.DataFrame:
        """
        使用 Twelve Data API 加载5分钟数据，然后随机选择1天
        """
        ...
        if not td_symbol:
            print(f"  [ERROR] 不支持的品种: {self.symbol}")
            return None
        ...
        if len(df) < 100:
            print(f"  [ERROR] 数据量不足 (仅{len(df)}根K线)")
            return None
        ...
        if len(available_dates) < 1:
            print(f"  [ERROR] 没有可用的日期")
            return None

        # 选择有足够数据的日期（至少50根K线，约4小时）
        valid_dates = []
        for date in available_dates:
            day_data = df[df['date'] == date]
            if len(day_data) >= 50:  # 至少50根5分钟K线
                valid_dates.append(date)

        if not valid_dates:
            print(f"  [ERROR] 没有足够数据的日期")
            return None
        ...
        print(f"  [OK] 随机选择日期: {selected_date}")
        print(f"  [OK] 该天数据: {len(day_df)} 根5分钟K线")
        print(f"  [OK] 时间范围: {day_df.index[0]} ~ {day_df.index[-1]}")
    ...
    def check_favorable_factors(self, signal: dict, idx: int) -> dict:
        ...
        # 检查大周期均线组是否顺趋势（5分钟数据，使用较短周期）
        ema_30 = self.df['ema_30'].iloc[idx] if idx >= 30 else None
        ema_50 = self.df['ema_50'].iloc[idx]
        close = self.df['Close'].iloc[idx]
        
        if ema_30 is not None:
            if signal['direction'] == 'long':
                if close > ema_30 and ema_30 > ema_50:
                    factors['score'] += 0.3
                    factors['reasons'].append('均线组多头排列')
            else:
                if close < ema_30 and ema_30 < ema_50:
                    factors['score'] += 0.3
                    factors['reasons'].append('均线组空头排列')
    ...
    def check_bull_signal(self, idx: int) -> dict | None:
        ...
        for i in range(idx - 1, max(idx - 10, 5), -1):
            if self.df['High'].iloc[i] < self.df['High'].iloc[i+1]:
                for j in range(i, min(i + 8, len(self.df) - 1)):
    ...
    def run_backtest(self, signals: list, generate_charts: bool = True) -> dict:
        ...
        for trade_num, signal in enumerate(signals, 1):
            ...
            if exit_price is not None and entry_price is not None:
                ...
                if generate_charts and trade_num <= 10:  # 只为前10笔交易生成图表
                    try:
    ...
    def plot_trade_chart(self, trade_info: dict, trade_num: int) -> str:
        ...
        end_idx = min(len(self.df), trade_info['exit_idx'] + 20)
        ...
        for i in range(len(chart_data)):
            ...
            ax1.add_patch(plt.Rectangle((mdates.date2num(date) - 0.0005, min(open_price, close)),
                                        0.001, abs(close - open_price),
        ...
        date_str = self.selected_date.strftime('%Y-%m-%d') if self.selected_date else 'N/A'
        ax1.set_title(f"{self.symbol.upper()} [5分钟] {date_str} - 交易#{trade_num} - {trade_info['direction'].upper()} - "
        ...
        ax2.set_xlabel('时间 (5分钟)', fontsize=11)
        ...
        # 保存图片
        date_str = self.selected_date.strftime('%Y%m%d') if self.selected_date else 'unknown'
        filename = f"{self.symbol}_{date_str}_{trade_num:03d}_{trade_info['direction']}.png"
...
def run_random_day_backtest():
    ...
    RISK_PER_TRADE = 0.02
    CHART_OUTPUT_DIR = "backtest_charts_5min_random"

    print("="*80)
    print("基于交易系统规则的5分钟数据回测 - 随机1天")
    print("="*80)
    ...
    for name, symbol in SYMBOLS.items():
        ...
        backtester = TradingSystem5MinBacktester(
            symbol=symbol,
        ...
        df = backtester.load_data_random_day(TWELVEDATA_API_KEY)
        ...
        if result:
            results.append({'symbol': name, 'date': backtester.selected_date, **result})

            return_pct = ((result['final_capital'] - INITIAL_CAPITAL) / INITIAL_CAPITAL * 100)
            
            print(f"\n--- {name} 回测结果 (日期: {backtester.selected_date}) ---")
    ...
    print("各品种汇总对比")
    print("="*80)
    print(f"{'品种':<10} {'日期':<12} {'交易数':>8} {'胜率':>8} {'TP1率':>8} {'总盈亏':>12} "
    ...
    for r in results:
        ...
        print(f"{r['symbol']:<10} {date_str:<12} {r['total_trades']:>8} {r['win_rate']:>7.1f}%"
...
if __name__ == "__main__":
    try:
        results = run_random_day_backtest()
